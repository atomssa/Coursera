...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    Seq(Layer(Temperatures, colorScaleTemp, 1975 to 2015),
      Layer(Deviations, colorScaleDev, 1980 to 2015))
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Int]): Signal[Int] = {
    Signal{
      val b = yearBounds(selectedLayer)()
      val s = sliderValue()
      if (s < b.start) b.start else if (s > b.end) b.end else s
    }
  }
